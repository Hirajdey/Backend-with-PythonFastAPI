...
async def update_password(user: user_dependency, db: db_dependency, user_verification: UserVerification):
    ...
    if not bcrypt_context.verify(user_verification.password, user_obj.hashed_password):
        raise HTTPException(status_code=401, detail="Error on password change")
    
    user_obj.hashed_password = bcrypt_context.hash(user_verification.new_password)
    db.add(user_obj)
    db.commit()
